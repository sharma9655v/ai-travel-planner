...
All routes are JSON. Errors return `{ "error": string }`.

### `POST /api/generate` — Generate an itinerary

```bash
curl -X POST http://localhost:3000/api/generate \
  -H "Content-Type: application/json" \
  -d '{
    "tripDetails": { "destination": "Kyoto", "departureDate": "2026-10-01", "returnDate": "2026-10-08" },
    "budget": { "totalBudget": 1500, "currency": "USD" },
    "travelers": { "groupSize": 2, "tripType": "couple" },
    "style": { "travelStyle": "culture", "travelPace": "moderate" },
    "interests": { "interests": ["temples", "food", "photography"] }
  }'
```

→ `200` — `{ id, message, itinerary, createdAt }`

### `POST /api/refine` — Surgical edit

```bash
curl -X POST http://localhost:3000/api/refine \
  -H "Content-Type: application/json" \
  -d '{ "itinerary": { ... }, "command": "Add a hidden gem near day 2" }'
```

→ `200` — `{ itinerary, summary, changed }` — unchanged sections are preserved **verbatim** by a merge-preserving diff.
...
curl -X POST http://localhost:3000/api/chat \
  -H "Content-Type: application/json" \
  -d '{ "message": "What should I pack for Kyoto in October?", "history": [] }'
```

→ `200` — `{ reply }`

### `GET /api/weather` — Live forecast

```
GET /api/weather?lat=35.0116&lon=135.7681&days=7
```

→ `200` — `{ current, forecast[], alerts[], updatedAt }` — days clamped to 1–16; coordinates validated server-side.

### `POST /api/share` — Publish a share link

```bash
curl -X POST http://localhost:3000/api/share \
  -H "Content-Type: application/json" \
  -d '{ "tripId": "abc123", "mode": "view", "itinerary": { ... } }'
```

→ `200` — `{ token, revokeKey, mode, createdAt, updatedAt }` (48-hex secrets; the revoke key is **never stored** — only its SHA-256 hash).

### `DELETE /api/share/[token]` — Revoke

```bash
curl -X DELETE http://localhost:3000/api/share/<token> \
  -H "Content-Type: application/json" \
  -d '{ "revokeKey": "<your-revoke-key>" }'
```
